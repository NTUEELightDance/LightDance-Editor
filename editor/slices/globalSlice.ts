    ...
    reducers: {
        ...
        editCurrentStatusLED: (
            ...
            if (typeof alpha === "number")
                (state.currentStatus[dancerName][partName] as LED).alpha =
                    alpha;
        },
        ...
        saveCurrentPos: (state) => {
            if (state.mode === EDIT) {
                ...
                state.posMap[state.posRecord[frameIndex]].pos =
                    state.currentPos;
                // const data = {
        ...
        setTime: (
            state,
            action: PayloadAction<{ from: string; time: number }>
        ) => {
            ...
            if (newPosFrame === state.posRecord.length - 1) {
                // can't interpolation
                state.currentPos =
                    state.posMap[state.posRecord[newPosFrame]].pos;
            } else {
                // do interpolation
        ...
        setEffectRecordMap: (
            state,
            action: PayloadAction<EffectRecordMapType>
        ) => {
        ...
        setEffectStatusMap: (
            state,
            action: PayloadAction<EffectStatusMapType>
        ) => {
        ...
        deleteEffect: (state, action: PayloadAction<string>) => {
            ...
            const effectFrameId: EffectRecordType =
                state.effectRecordMap[effectName];
        ...
        applyEffect: (state, action: PayloadAction<string>) => {
            ...
            const shiftTime: number =
                state.timeData.time -
                state.effectStatusMap[state.effectRecordMap[effectName][0]]
                    .start;
