...
def _binary_search_with_lr(
    ...
    :param arr: sorted list of integers
    :param x: integer to search for
    :return: (index of the first number in arr that is larger than or equal to x,
              index of the last number in arr that is less than or equal to x)
              or out of range error: showing that the number is bigger/smaller than all num. in array
    """
    l, r = 0, len(arr) - 1
    if x > arr[r]:
...
# find the smallest cont. range in arr that include [left, right] which is in the form of indexes
