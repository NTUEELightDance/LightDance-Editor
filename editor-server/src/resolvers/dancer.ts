...
import { AddDancerInput, editDancerInput, deleteDancerInput } from './inputs/dancer';
...
export class DancerResolver {
    ...
    @Mutation(returns => DancerResponse)
    async editDancer(@Arg("dancer") newDancerData: editDancerInput, @Ctx() ctx: any) {
        const { id, name } = newDancerData
        const newDancer = await ctx.db.Dancer.findOneAndUpdate({ id }, { name }, { new: true }).populate('parts').populate('positionData')
        console.log(newDancer)
        if (newDancer) {
            return Object.assign(newDancer, { ok: true })
        }
        return Object.assign({ name: "", parts: [], positionData: [], id: "" }, { ok: false, msg: "dancer doesn't exist" })
    }

    @Mutation(returns => DancerResponse)
    async deleteDancer(@Arg("dancer") newDancerData: deleteDancerInput, @Ctx() ctx: any) {
        const { id } = newDancerData
        const dancer = await ctx.db.Dancer.findOne({ id })
        if (dancer) {
            await Promise.all(dancer.parts.map(async (ref: string) => {
